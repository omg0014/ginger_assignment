...
app.get('/api/jobs/:id/result', async (req, res) => {
    ...
    if (job.status === 'PENDING' || job.status === 'PROCESSING')
        return res.status(202).json({ status: 'processing', message: 'analysis in progress.' });
    if (job.status === 'FAILED')
        return res.status(404).json({ jobId: job.id, status: 'failed', failureReason: job.failureReason, checks: [] });
...
app.get('/api/history', async (req, res) => {
    ...
    if (req.query.search) {
        where.OR = [
            ...
            { analysis: { numberPlate: { contains: req.query.search } } }
        ];
    }
    ...
    res.json({
        data: jobs.map(j => ({
            ...
            processingTimeMs: j.analysis?.processingTimeMs
        })),
        total, page, limit, totalPages: Math.ceil(total / limit)
    });
});
app.get('/api/analytics/summary', async (req, res) => {
    const [totalJobs, highRiskCount, completedCount, pendingCount] = await Promise.all([
        prisma.job.count(),
  ...
  `;
    res.json({
        totalJobs, highRiskCount, completedCount, pendingCount,
        trends: dailyData.map((d) => ({ date: d.date.toISOString().split('T')[0], count: Number(d.count) })),
        breakdown: []
    });
});
export default app;
