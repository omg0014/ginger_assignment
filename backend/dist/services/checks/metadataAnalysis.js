...
export async function metadataAnalysis(filePath) {
    let exif = {};
    try {
        exif = await exifr.parse(filePath, { all: true });
    }
    catch { }
    const signals = [];
    let score = 0;
    ...
    if (editingFound) {
        signals.push(`editing software detected: ${editingFound}`);
        score += 0.5;
    }
    if (!exif?.latitude && !exif?.longitude) {
        signals.push('no gps coordinates');
        score += 0.15;
    }
    if (!exif?.Make && !exif?.Model) {
        signals.push('no camera make/model');
        score += 0.2;
    }
    if (exif?.ModifyDate && exif?.CreateDate) {
        const diff = Math.abs(new Date(exif.ModifyDate).getTime() - new Date(exif.CreateDate).getTime());
        if (diff > 60000) {
            signals.push('file modified after creation');
            score += 0.35;
        }
    }
